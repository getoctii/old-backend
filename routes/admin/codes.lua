...
function Codes:GET()
  ...
  local page = self.params.last_code_id and
    db.query('SELECT * FROM (SELECT *, RANK() OVER (order by created_at desc) rank FROM "codes" WHERE order by created_at desc) t WHERE rank > (SELECT rank FROM (SELECT *, RANK() OVER (order by created_at desc) rank FROM codes WHERE id = ?) t2) LIMIT 25', self.params.last_code_id)
